...
class PortfolioAnalyzer:
    def __init__(self, positions, start_date):
        ...
        self.insider_trades = self._load_insider_trades()
    ...
    def graph_portfolio(self) -> go.Figure:
        ...
        # Add vertical lines for latest insider transaction per ticker (if available)
        palette = px.colors.qualitative.Plotly
        for idx, ticker in enumerate(self.tickers):
            trade_date = self.insider_trades.get(ticker)
            ...
            fig.add_vline(
                ...
                line_width=2,
                line_dash="dash",
                annotation_text=f"{ticker} trade",
    ...
    def graph_individual_stocks(self) -> go.Figure:
        ...
        for idx, ticker in enumerate(self.tickers):
            ...
            # Add vertical line for latest insider transaction date for this ticker, if available
            trade_date = self.insider_trades.get(ticker)
    ...
    def _process_data(self) -> tuple:
        ...
        # Check if we have a MultiIndex columns (common in new yfinance if multiple tickers)
        # However, the subsequent code expects specific column access.
        # If yfinance returns a MultiIndex (Price, Ticker), we might need to handle it.
        # For now, let's inspect the structure.
        
        # If specific column names are expected like 'Close', 'Open' directly or as top level:
        try:
            close = data["Close"]
            open_ = data['Open']
            low = data['Low']
            ...
            # Fallback if structure is different (e.g. sometimes it's (Ticker, Price) or vice versa)
             # But usually 'Close' is a top level key if grouped_by='column' (default)
             pass
        ...
        if len(tickers) == 1:
            # Convert to DataFrame only when a Series was returned; otherwise leave as-is.
    ...
    def _load_insider_trades(self) -> dict:
        """
        Load insider transaction dates for the tickers in the portfolio.

        Returns:
            dict mapping ticker -> latest transaction_date (Timestamp)
        """
        try:
            df = pd.read_csv(
                'data/insider_trades_with_returns.csv',
                usecols=['ticker', 'transaction_date'],
            ...
            for ticker in self.positions.keys():
                t_df = df[df['ticker'] == ticker.upper()]
                if t_df.empty:
                    continue

                # Prefer trades within the analysis window; otherwise fall back to the latest overall
                in_range = t_df[(t_df['transaction_date'] >= start) &
                                (t_df['transaction_date'] <= now)]
                if not in_range.empty:
                    trades[ticker.upper()] = in_range['transaction_date'].max()
                else:
                    trades[ticker.upper()] = t_df['transaction_date'].max()

            return trades
        except Exception as e:  # pragma: no cover - diagnostic logging only
            print(f"Could not load insider trades: {e}")
            return {}
...
"""
TickerAutofill helper disabled per request.
